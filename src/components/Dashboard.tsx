            ...
            <div className="container" style={{ maxWidth: 1200 }}>
                ...
                <div style={{ marginBottom: "var(--spacing-xl)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <div style={{ display: "flex", gap: "12px" }}>
                            <Link to="/projects" className="btn btn-primary" style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{
                    ...
                    <div style={{ marginBottom: "24px" }}>
                        <h3 style={{ margin: 0, fontSize: "20px", fontWeight: 600, display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{
                        ...
                        <QuickLink
                            ...
                            desc="Track quote statuses and manage the sales lifecycle."
...
const StatCard = ({ label, value, icon }: { label: string, value: string, icon: React.ReactNode }) => (
    <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px", color: "var(--color-text-muted)" }}>
            {icon}
            <span style={{ fontSize: "13px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.02em" }}>{label}</span>
        </div>
        <div style={{ fontSize: "24px", fontWeight: 700, color: "var(--color-text)" }}>{value}</div>
    </div>
);

const QuickLink = ({ to, title, desc, icon }: { to: string, title: string, desc: string, icon: React.ReactNode }) => (
    <Link to={to} style={{
        textDecoration: "none",
        padding: "24px",
        borderRadius: "14px",
        ...
        <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, color: "var(--color-text)", fontSize: "16px" }}>{title}</div>
            <div style={{ fontSize: "13px", color: "var(--color-text-muted)", marginTop: "4px", lineHeight: 1.4 }}>{desc}</div>
        </div>
