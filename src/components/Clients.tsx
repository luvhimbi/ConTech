...
const Clients: React.FC = () => {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setForm({ name: "", email: "", address: "", phone: "", tags: "", notes: "", preferredContact: "", siteAccessRules: "" });
            setShowForm(false);
    ...
    if (loading) return <div style={loaderWrapper}><Loader2 className="animate-spin" size={32} color="var(--color-primary)" /></div>;
            ...
            <div className="container" style={{ maxWidth: 1400 }}>
                ...
                <div style={headerLayout}>
                    <div>
                        ...
                        <p style={{ color: '#64748b', fontSize: 14, marginTop: 4 }}>Manage {filtered.length} business contacts</p>
                    </div>
                    ...
                    <div style={drawerOverlay}>
                        <div style={drawerContent}>
                            <div style={drawerHeader}>
                                ...
                                <button onClick={() => setShowForm(false)} style={closeBtn}><X size={24}/></button>
                            </div>
                            ...
                            <form onSubmit={handleCreate} style={drawerBody}>
                                <div style={formSection}>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                                        ...
                                        <div className="form-group">
                                            ...
                                            <input className="form-control" placeholder="WhatsApp" value={form.preferredContact} onChange={e => setForm({...form, preferredContact: e.target.value})} />
                                        </div>
                                    </div>
                                    <div className="form-group">
                                ...
                                <div style={formSection}>
                                    ...
                                    <div className="form-group">
                                        ...
                                        <input className="form-control" placeholder="VIP, Contractor, Late-Payer" value={form.tags} onChange={e => setForm({...form, tags: e.target.value})} />
                                    </div>
                                    <div className="form-group">
                                        <label className="form-label">Site Access Rules</label>
                                        <textarea className="form-control" rows={3} placeholder="Gate codes, PPE requirements..." value={form.siteAccessRules} onChange={e => setForm({...form, siteAccessRules: e.target.value})} />
                                    </div>
                                    <div className="form-group">
                        ...
                        {filtered.map((c) => (
                            <tr key={c.id} className="client-row" style={trStyle}>
                                <td style={tdStyle}>
                                    <Link to={`/clients/${c.id}`} style={nameLink}>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            ...
                                            <span style={{ fontSize: 11, color: '#94a3b8', textTransform: 'uppercase' }}>ID: {c.id?.slice(-6)}</span>
                                        </div>
                                    </Link>
                                </td>
                                <td style={tdStyle}>
                                    <div style={contactStack}>
                                        <div style={contactLine}><Mail size={12}/> {c.email}</div>
                                        {c.phone && <div style={contactLine}><Phone size={12}/> {c.phone}</div>}
                                    </div>
                                </td>
                                <td style={tdStyle}>
                                    <div style={tagStack}>
                                        {c.tags?.slice(0, 2).map((t, i) => (
                                            <span key={i} style={miniTag}>{t}</span>
                                        ))}
                                ...
                                <td style={tdStyle}>
                                    <div style={siteCell}>
                                        {c.address ? (
                                            <div style={contactLine}><MapPin size={12}/> {c.address.slice(0, 25)}...</div>
                                        ) : <span style={{ color: '#cbd5e1', fontSize: 12 }}>No address</span>}
                                        {c.siteAccessRules && <div style={contactLine}><Shield size={12} color="#10b981"/> Access Rules Set</div>}
                                    </div>
                                </td>
                                <td style={{ ...tdStyle, textAlign: 'right' }}>
                                    <div style={actionRow}>
                                        <button onClick={() => handleDelete(c.id!)} style={iconBtn} className="del-btn"><Trash2 size={16}/></button>
                                        <Link to={`/clients/${c.id}`} style={iconBtn}><ExternalLink size={16}/></Link>
                                    </div>
                                </td>
                            </tr>
                        ))}
